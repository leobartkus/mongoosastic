...
module.exports = function Mongoosastic(schema, options){
  var mapping = getMapping(schema)
    ...
    , esClient  = options.client_config ? new elasticsearch.Client(options.client_config) : new elasticsearch.Client({host: {host: host, port: port, protocol: protocol, auth: auth}})
