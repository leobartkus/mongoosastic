...
function getCleanTree(tree, paths, prefix) {
  ...
  for (var field in tree){
    ...
    if (type) {
      ...
      } else if (isMongooseSchemaType(value)){
        //don't modify mongoose schema type singletons
        cleanTree[field] = {};
        cleanTree[field].type = type;
      } else {
...
//
// Returns true if the object is a mongoose schema type
//
// @param obj
// @return true or false
//
function isMongooseSchemaType(obj)
{
    for(var t in mongoose.Schema.Types) {
        if(mongoose.Schema.Types[t] === obj)
        {
            return true;
        }
    }
